...
class nagios::plugins::puppet {
  ...
  nagios::config::nrpe { 'check_puppet_checkin_db': command => 'check_puppet_checkin_db', 
  }

  # check_puppetmaster
  nagios::config::nrpe { 'check_puppetmaster': command => 'check_daemons puppet\ master', 
  }

  # check_puppet
  nagios::config::nrpe { 'check_puppet': command => 'check_daemons puppet\ agent', 
  }

  # check_puppet_environment
  nagios::plugin { 'check_puppet_environment': }

  nagios::config::nrpe { 'check_puppet_environment': command => 'check_puppet_environment', 
  }
}
